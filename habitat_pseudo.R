...
habitats <- as.factor(rep(habitat_names, each = pseudos_per_habitat))
pseudos <- cbind(habitats, pseudoquad_data_wde)
write.csv(pseudos, "data/pseudos.csv")
...
for(ash_quad in 1:nrow(ash_pred)){
  ...
  for(habitat in 1:no_of_habitats){
    ...
    dist <- sqrt(dist)
    dist_to_habitats <- rbind(dist_to_habitats,
                              data.frame(quad=this_quad,
                                         habitat_nam=habitat_umap$habitat[habitat],
                                         distance=dist))
  }
  # Similarity score
  invsum_dist <- 0
